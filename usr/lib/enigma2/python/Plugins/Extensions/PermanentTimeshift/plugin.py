...
class InfoBar(InfoBarOrg):
	...
	def __evSOF(self):
		if not config.plugins.pts.enabled.value or not self.timeshiftEnabled():
			return
	...
	def __seekableStatusChanged(self):
		enabled = False
		if not self.isSeekable() and self.timeshiftEnabled():
			enabled = True
		self["TimeshiftActivateActions"].setEnabled(enabled)

		enabled = False
		if config.plugins.pts.enabled.value and config.plugins.pts.showinfobar.value and self.timeshiftEnabled() and self.isSeekable():
			enabled = True

		self["PTSSeekPointerActions"].setEnabled(enabled)

		# Reset Seek Pointer And Eventname in InfoBar
		if config.plugins.pts.enabled.value and config.plugins.pts.showinfobar.value and self.timeshiftEnabled() and not self.isSeekable():
		...
		if config.plugins.pts.enabled.value and self.timeshiftEnabled() and not self.isSeekable():
	...
	def stopTimeshift(self):
		...
		if config.plugins.pts.enabled.value and self.timeshiftEnabled():
			if self.isSeekable():
	...
	def stopTimeshiftConfirmed(self, confirmed, switchToLive=True):
		was_enabled = self.timeshiftEnabled()
	...
	def ptsSeekPointerOK(self):
		if self.pts_pvrStateDialog == "PTSTimeshiftState" and self.timeshiftEnabled() and self.isSeekable():
	...
	def ptsSeekPointerReset(self):
		if self.pts_pvrStateDialog == "PTSTimeshiftState" and self.timeshiftEnabled():
	...
	def ptsSeekPointerSetCurrentPos(self):
		if not self.pts_pvrStateDialog == "PTSTimeshiftState" or not self.timeshiftEnabled() or not self.isSeekable():
			return
	...
	def ptsLengthCheck(self):
		...
		# Length Check
		if config.plugins.pts.enabled.value and self.session.screen["Standby"].boolean is not True and self.timeshiftEnabled() and (time() - self.pts_starttime) >= (config.plugins.pts.maxlength.value * 60):
			if self.save_current_timeshift:
...
def keyNumberGlobal(self, number):
	if self.pts_pvrStateDialog == "PTSTimeshiftState" and self.timeshiftEnabled() and self.isSeekable() and number == 0:
		InfoBarTimeshiftState._mayShow(self)
	...
	if number and config.plugins.pts.enabled.value and self.timeshiftEnabled() and not self.isSeekable():
...
############################
def _mayShow(self):
	if InfoBar and InfoBar.instance and self.execing and self.timeshiftEnabled() and self.isSeekable():
